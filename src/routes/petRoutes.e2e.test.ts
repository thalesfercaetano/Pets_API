...
describe('E2E /pets', () => {
  ...
  describe('POST /pets', () => {
    it('cria pet com sucesso e retorna 201', async () => {
      ...
      const [instId] = await db('INSTITUICOES').insert({ nome: 'ONG Pets', email: 'ong@pets.com' });
  ...
  describe('GET /pets/:id', () => {
    it('retorna 200 com pet existente', async () => {
      if (!dbReady) { console.warn('DB indisponível; ignorando teste'); return; }
      const [instId] = await db('INSTITUICOES').insert({ nome: 'ONG A', email: 'a@ong.com' });
      const [petId] = await db('PETS').insert({ nome: 'Rex', especie: 'Cachorro', sexo: 'M', instituicao_id: Number(instId), status_adocao: 'disponível' });
  ...
  describe('PATCH /pets/:id', () => {
    it('atualiza pet e retorna 200', async () => {
      if (!dbReady) { console.warn('DB indisponível; ignorando teste'); return; }
      const [instId] = await db('INSTITUICOES').insert({ nome: 'ONG B', email: 'b@ong.com' });
      const [petId] = await db('PETS').insert({ nome: 'Luna', especie: 'Gato', sexo: 'F', instituicao_id: Number(instId), status_adocao: 'disponível' });
      const res = await request.patch(`/pets/${petId}`).send({ name: 'Luna Atualizada' });
      expect(res.status).toBe(200);
  ...
  describe('DELETE /pets/:id', () => {
    it('deleta pet e retorna 200', async () => {
      if (!dbReady) { console.warn('DB indisponível; ignorando teste'); return; }
      const [instId] = await db('INSTITUICOES').insert({ nome: 'ONG C', email: 'c@ong.com' });
      const [petId] = await db('PETS').insert({ nome: 'Bolt', especie: 'Cachorro', sexo: 'M', instituicao_id: Number(instId), status_adocao: 'disponível' });
